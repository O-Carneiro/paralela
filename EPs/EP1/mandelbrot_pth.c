...
void compute_mandelbrot(){
    //divide the work between NUM_THREADS threads.
    pthread_t tids[NUM_THREADS];
    thread_data thread_args[NUM_THREADS];
    int rows_per_thread = i_y_max / NUM_THREADS;
    
    for(int i = 0; i < NUM_THREADS; i++){
        thread_args[i].y_start = rows_per_thread*i;
        if(i == NUM_THREADS-1) 
            thread_args[i].y_end = i_y_max;
        else
            thread_args[i].y_end = thread_args[i].y_start + rows_per_thread;
        if(pthread_create(&tids[i], NULL, compute_mandelbrot_thread, (void *)&thread_args[i])!=0)
            perror("Falha ao criar thread:");
    }

    for(int i =  0; i < NUM_THREADS; i++){
        if(pthread_join(tids[i], NULL))
            perror("Falha ao 'joinar' thread:");
    }
}
