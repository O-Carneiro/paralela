...
arg_pattern = re.compile(r"Performance counter stats for './mandelbrot_seq [-\d.]+ [-\d.]+ [-\d.]+ [-\d.]+ (\d+)'")
time_pattern = re.compile(r"\s+(\d+,\d+) \+- (\d+,\d+) seconds time elapsed\s+\( \+-  (\d+,\d+)% \)")

# Directory containing the input files
input_dir = './results/mandelbrot_seq/'  # Update with the correct directory
output_dir = './mandelbrot_seq_IO/'  # Directory to save the CSV files
